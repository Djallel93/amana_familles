...
function processBulkUpdateRow(row) {
    ...
    if (updatedFields.length === 0) return { success: false, error: 'Au moins un champ doit être renseigné' };

    updateData.forceInProgress = true;

    const result = updateFamilyById(familyId, updateData);
    if (result.success) return { success: true, updatedFields, quartierWarning: result.quartierWarning };
...
function processBulkUpdate(batchSize = 10) {
    ...
    rowsToProcess.forEach(item => {
        ...
        try {
            ...
            const result = processBulkUpdateRow(row);
...
function resetUpdateProcessingStatus() {
    ...
    data.forEach((row, arrayIndex) => {
        if (row[0] && row[0].includes('En cours')) {
            sheet.getRange(FIRST_DATA_ROW + arrayIndex, BULK_UPDATE_COLUMNS.COMMENTAIRE + 1).setValue('En attente (reset après timeout)');
            resetCount++;
        }
    });
    if (resetCount > 0) logInfo(`${resetCount} lignes réinitialisées dans Bulk Update`);
}
